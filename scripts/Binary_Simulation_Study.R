...
saveRDS(ms.summ.30, file="binary_S1_DoppelNull_ms_summ_30.rds")
saveRDS(est.30.agg, file="binary_S1_DoppelNull_est_agg_30.rds")
saveRDS(est.long.30, file="binary_S1_DoppelNull_est_long_30.rds")
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.50, file="binary_S1_DoppelNull_ms_summ_50.rds")
saveRDS(est.50.agg, file="binary_S1_DoppelNull_est_agg_50.rds")
saveRDS(est.long.50, file="binary_S1_DoppelNull_est_long_50.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.1000, file="binary_S1_DoppelNull_ms_summ_1000.rds")
saveRDS(est.1000.agg, file="binary_S1_DoppelNull_est_agg_1000.rds")
saveRDS(est.long.1000, file="binary_S1_DoppelNull_est_long_1000.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.30, file="binary_S2_Null_ms_summ_30.rds")
saveRDS(est.30.agg, file="binary_S2_Null_est_agg_30.rds")
saveRDS(est.long.30, file="binary_S2_Null_est_long_30.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.50, file="binary_S2_Null_ms_summ_50.rds")
saveRDS(est.50.agg, file="binary_S2_Null_est_agg_50.rds")
saveRDS(est.long.50, file="binary_S2_Null_est_long_50.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.1000, file="binary_S2_Null_ms_summ_1000.rds")
saveRDS(est.1000.agg, file="binary_S2_Null_est_agg_1000.rds")
saveRDS(est.long.1000, file="binary_S2_Null_est_long_1000.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.30, file="binary_S4_Nullout_ms_summ_30.rds")
saveRDS(est.30.agg, file="binary_S4_Nullout_est_agg_30.rds")
saveRDS(est.long.30, file="binary_S4_Nullout_est_long_30.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.50, file="binary_S4_Nullout_ms_summ_50.rds")
saveRDS(est.50.agg, file="binary_S4_Nullout_est_agg_50.rds")
saveRDS(est.long.50, file="binary_S4_Nullout_est_long_50.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.1000, file="binary_S4_Nullout_ms_summ_1000.rds")
saveRDS(est.1000.agg, file="binary_S4_Nullout_est_agg_1000.rds")
saveRDS(est.long.1000, file="binary_S4_Nullout_est_long_1000.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
coef.name <- c("beta_0","beta_1","beta_3","beta_4","lambda_0","lambda_1","lambda_2","lambda_3")
true.coef <- c(beta_0, beta_1, beta_3, beta_4, lambda_0, lambda_1, lambda_2, lambda_3)
...
# Draw random data
simdata.30 <- lapply(simdata.30,function(r) {
  x <- rbinom(n_obs.30,1,0.5)
  z <- rbinom(n_obs.30,1,0.5)
  xz <- x*z
  normal <- rnorm(n_obs.30,mean=0,sd=sigma)
  u <- sqrt(exp(lambda_0 + lambda_1*x + lambda_2*z + lambda_3*x*z))*normal
  y <- beta_0+x*beta_1 + z*beta_3 + x*z*beta_4 + u
  random <- .Random.seed #store RandomNumberstate after each repetition
  list(y=y, x=x, z=z, xz=xz, random=random)
})
...
two.step.fit.30 <- vector("list", length=n_sim) #preallocate to save calculation time
two.step.fit.30 <- lapply(simdata.30, function(data){
  lm.fit <-  tryCatch(lm(formula=data$y~data$x + data$z + data$xz),
                      error=function(e) list(as.character(e))) #fit lm model of the mean and save error if impossible
  resid.lm <- residuals(lm.fit)^2
...
ml.fit.30 <- matrix(data=NA,nrow=n_sim) #preallocate to save calculation time
ml.fit.30 <- lapply(simdata.30, function(data){
  tryCatch(dglm(formula=data$y~data$x + data$z + data$xz,dformula = ~ data$x +  data$z + data$xz, family = gaussian(link = "identity"), dlink = "log", method="ml"),
           error=function(e) list(as.character(e))) #fit ml model for the variance function regression and save error if impossible
})
#check Fit
ml.fit.na.30 <- !sapply(ml.fit.30,is.model.fun,legalmod)
...
reml.fit.30 <- matrix(data=NA,nrow=n_sim) #preallocate to save calculation time
reml.fit.30 <- lapply(simdata.30, function(data){
  tryCatch(dglm(formula=data$y~data$x + data$z + data$xz,dformula = ~ data$x +  data$z + data$xz, family = gaussian(link = "identity"), dlink = "log", method="reml"),
           error=function(e) list(as.character(e))) #fit reml model for the variance function regression and save error if impossible
})
#check Fit
reml.fit.na.30 <- !sapply(reml.fit.30,is.model.fun,legalmod)
...
# Extract coeficients from the models if possible, otherwise set to NA
two.step.coef.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.coef.30) <- coef.name
two.step.coef.30[,1:4] <- t(sapply(lapply(two.step.fit.30,getElement,"lm.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
...
two.step.coef.30[,5:8] <- t(sapply(lapply(two.step.fit.30,getElement,"gamma.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
ml.coef.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.coef.30) <- coef.name
ml.coef.30[,1:4] <- t(sapply(ml.fit.30,function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
ml.coef.30[,5:8] <- t(sapply(lapply(ml.fit.30,getElement,"dispersion.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.coef.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.coef.30) <- coef.name
reml.coef.30[,1:4] <- t(sapply(reml.fit.30,function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.coef.30[,5:8] <- t(sapply(lapply(reml.fit.30,getElement,"dispersion.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
two.step.se.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.se.30) <- coef.name
two.step.se.30[,1:4] <- t(vapply(lapply(two.step.fit.30,getElement,"lm.fit"), FUN.VALUE=c(1.1,1.1,1.1,1.1), function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit)))[1:4,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set se to NA if model could not be estimated
  }
}))
two.step.se.30[,5:8] <- t(sapply(lapply(two.step.fit.30,getElement,"gamma.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit, dispersion=2)))[1:4,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set se to NA if model could not be estimated
  }
}))

ml.se.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.se.30) <- coef.name
ml.se.30[,1:4] <- t(sapply(1:length(ml.fit.30),function(fit){
  if(!any(is.na(ml.coef.30[fit,]))){
    as.data.frame(coef(summary(ml.fit.30[[fit]]), dispersion=1))[1:4,"Std. Error"]
  }else{
    as.numeric(c(NA,NA,NA,NA)) #set coef to NA if model could not be estimated
  }
}))
ml.se.30[,5:8] <- t(sapply(lapply(ml.fit.30,getElement,"dispersion.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit, dispersion=2)))[1:4,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.se.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.se.30) <- coef.name
reml.se.30[,1:4] <- t(sapply(reml.fit.30,function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit), dispersion=1))[1:4,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
  }
}))
reml.se.30[,5:8] <- t(sapply(lapply(reml.fit.30,getElement,"dispersion.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit, dispersion=2)))[1:4,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
two.step.p.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.p.30) <- coef.name
two.step.p.30[,1:4] <- t(sapply(lapply(two.step.fit.30,getElement,"lm.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit)))[1:4,"Pr(>|t|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
  }
}))
two.step.p.30[,5:8] <- t(sapply(lapply(two.step.fit.30,getElement,"gamma.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit, dispersion=2)))[1:4,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
  }
}))

ml.p.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.p.30) <- coef.name
ml.p.30[,1:4] <- t(sapply(1:length(ml.fit.30),function(fit){
  if(!any(is.na(ml.coef.30[fit,]))){
    as.data.frame(coef(summary(ml.fit.30[[fit]], dispersion=1)))[1:4,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
  }
}))
ml.p.30[,5:8] <- t(sapply(lapply(ml.fit.30,getElement,"dispersion.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit, dispersion=2)))[1:4,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.p.30 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.p.30) <- coef.name
reml.p.30[,1:4] <- t(sapply(reml.fit.30,function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit, dispersion=1)))[1:4,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
  }
}))
reml.p.30[,5:8] <- t(sapply(lapply(reml.fit.30,getElement,"dispersion.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    as.data.frame(coef(summary(fit, dispersion=2)))[1:4,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
est.30 <- merge(coef.30, se.30, by=c("repetition", "model"),suffixes=c(".coef",".se"), sort=FALSE)
est.30 <- merge(est.30, p.30, by=c("repetition", "model"), sort=FALSE)
colnames(est.30)[colnames(est.30) %in% coef.name] <- c("beta_0.p","beta_1.p","beta_3.p","beta_4.p","lambda_0.p","lambda_1.p","lambda_2.p","lambda_3.p")

# est.long.30 <- reshape(est.30, idvar=c("repetition","model"), direction ="long",
#                        varying = list(coef=c(3:10), se=c(10:18), p=c(19:26)),
#                        sep =".",
#                        v.names = c("coef","se","p"),
#                        timevar = "coef.type",
#                        times = coef.name
...
est.long.30 <- est.30 %>%
  pivot_longer(
    cols = ends_with(".coef") | ends_with(".se") | ends_with(".p"),
    names_to = c("coef.type", ".value"),
    names_sep = "\\."
  )
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.30, file="binary_S6_Exp_ms_summ_30.rds")
saveRDS(est.long.30, file="binary_S6_Exp_est_long_30.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# Extract coeficients from the models if possible, otherwise set to NA
two.step.coef.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.coef.50) <- coef.name
two.step.coef.50[,1:4] <- t(sapply(lapply(two.step.fit.50,getElement,"lm.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
two.step.coef.50[,5:8] <- t(sapply(lapply(two.step.fit.50,getElement,"gamma.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
ml.coef.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.coef.50) <- coef.name
ml.coef.50[,1:4] <- t(sapply(ml.fit.50,function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
ml.coef.50[,5:8] <- t(sapply(lapply(ml.fit.50,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.coef.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.coef.50) <- coef.name
reml.coef.50[,1:4] <- t(sapply(reml.fit.50,function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.coef.50[,5:8] <- t(sapply(lapply(reml.fit.50,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
two.step.se.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.se.50) <- coef.name
two.step.se.50[,1:4] <- t(sapply(lapply(two.step.fit.50,getElement,"lm.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
...
two.step.se.50[,5:8] <- t(sapply(lapply(two.step.fit.50,getElement,"gamma.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
...
ml.se.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.se.50) <- coef.name
ml.se.50[,1:4] <- t(sapply(ml.fit.50,function(fit){ 
  if(is.model.fun(fit,legalmod)){
...
ml.se.50[,5:8] <- t(sapply(lapply(ml.fit.50,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.se.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.se.50) <- coef.name
reml.se.50[,1:4] <- t(sapply(reml.fit.50,function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit), dispersion=1)[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.se.50[,5:8] <- t(sapply(lapply(reml.fit.50,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
two.step.p.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.p.50) <- coef.name
two.step.p.50[,1:4] <- t(sapply(lapply(two.step.fit.50,getElement,"lm.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
...
two.step.p.50[,5:8] <- t(sapply(lapply(two.step.fit.50,getElement,"gamma.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
...
ml.p.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.p.50) <- coef.name
ml.p.50[,1:4] <- t(sapply(ml.fit.50,function(fit){
  if(is.model.fun(fit,legalmod)){
...
ml.p.50[,5:8] <- t(sapply(lapply(ml.fit.50,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.p.50 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.p.50) <- coef.name
reml.p.50[,1:4] <- t(sapply(reml.fit.50,function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=1))[,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
...
reml.p.50[,5:8] <- t(sapply(lapply(reml.fit.50,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
est.50 <- merge(coef.50, se.50, by=c("repetition", "model"),suffixes=c(".coef",".se"), sort=FALSE)
est.50 <- merge(est.50, p.50, by=c("repetition", "model"), sort=FALSE)
colnames(est.50)[colnames(est.50) %in% coef.name] <- c("beta_0.p","beta_1.p","beta_3.p","beta_4.p","lambda_0.p","lambda_1.p","lambda_2.p","lambda_3.p")
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.50, file="binary_S6_Exp_ms_summ_50.rds")
saveRDS(est.long.50, file="binary_S6_Exp_est_long_50.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
# Extract coeficients from the models if possible, otherwise set to NA
two.step.coef.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.coef.1000) <- coef.name
two.step.coef.1000[,1:4] <- t(sapply(lapply(two.step.fit.1000,getElement,"lm.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
two.step.coef.1000[,5:8] <- t(sapply(lapply(two.step.fit.1000,getElement,"gamma.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
ml.coef.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.coef.1000) <- coef.name
ml.coef.1000[,1:4] <- t(sapply(ml.fit.1000,function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
ml.coef.1000[,5:8] <- t(sapply(lapply(ml.fit.1000,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.coef.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.coef.1000) <- coef.name
reml.coef.1000[,1:4] <- t(sapply(reml.fit.1000,function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.coef.1000[,5:8] <- t(sapply(lapply(reml.fit.1000,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(fit)
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
two.step.se.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.se.1000) <- coef.name
two.step.se.1000[,1:4] <- t(sapply(lapply(two.step.fit.1000,getElement,"lm.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit))[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set se to NA if model could not be estimated
...
two.step.se.1000[,5:8] <- t(sapply(lapply(two.step.fit.1000,getElement,"gamma.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set se to NA if model could not be estimated
...
ml.se.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.se.1000) <- coef.name
ml.se.1000[,1:4] <- t(sapply(ml.fit.1000,function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit), dispersion=1)[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
ml.se.1000[,5:8] <- t(sapply(lapply(ml.fit.1000,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.se.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.se.1000) <- coef.name
reml.se.1000[,1:4] <- t(sapply(reml.fit.1000,function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit), dispersion=1)[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.se.1000[,5:8] <- t(sapply(lapply(reml.fit.1000,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Std. Error"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
two.step.p.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(two.step.p.1000) <- coef.name
two.step.p.1000[,1:4] <- t(sapply(lapply(two.step.fit.1000,getElement,"lm.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit))[,"Pr(>|t|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
...
two.step.p.1000[,5:8] <- t(sapply(lapply(two.step.fit.1000,getElement,"gamma.fit"),function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
...
ml.p.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(ml.p.1000) <- coef.name
ml.p.1000[,1:4] <- t(sapply(ml.fit.1000,function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=1))[,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
...
ml.p.1000[,5:8] <- t(sapply(lapply(ml.fit.1000,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
reml.p.1000 <- matrix(data=NA, nrow=n_sim,ncol=8)
colnames(reml.p.1000) <- coef.name
reml.p.1000[,1:4] <- t(sapply(reml.fit.1000,function(fit){
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=1))[,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set p-values to NA if model could not be estimated
...
reml.p.1000[,5:8] <- t(sapply(lapply(reml.fit.1000,getElement,"dispersion.fit"),function(fit){ 
  if(is.model.fun(fit,legalmod)){
    coef(summary(fit, dispersion=2))[,"Pr(>|z|)"]
  }else{
    c(NA,NA,NA,NA) #set coef to NA if model could not be estimated
...
est.1000 <- merge(coef.1000, se.1000, by=c("repetition", "model"),suffixes=c(".coef",".se"), sort=FALSE)
est.1000 <- merge(est.1000, p.1000, by=c("repetition", "model"), sort=FALSE)
colnames(est.1000)[colnames(est.1000) %in% coef.name] <- c("beta_0.p","beta_1.p","beta_3.p","beta_4.p","lambda_0.p","lambda_1.p","lambda_2.p","lambda_3.p")
...
# export data.frames for plotting and creating tables
saveRDS(ms.summ.1000, file="binary_S6_Exp_ms_summ_1000.rds")
saveRDS(est.long.1000, file="binary_S6_Exp_est_long_1000.rds")
# Delete everything except predefined variables and functions
rm(list=ls()[! (ls() %in% lsf.str() | ls() %in% c("legalmod"))])
...
simdata.30 <- lapply(simdata.30,function(r) {
  ...
  y <- beta_0+x*beta_1 + z*beta_3 + x*z*beta_4 + u
  random <- .Random.seed #store RandomNumberstate after each repetition
  list(y=y, x=x, z=z, z=z, xz=xz, random=random)
})
...
true_betamed <- c(beta_0, beta_1, beta_3, beta_4)
...
true_betatau <- true_betamed + t(qnorm(tau, 0, 1) %*% t(c(lambda_0, lambda_1, lambda_2, lambda_3)))
true_betatau <- data.frame(true_betatau, row.names = c("(Intercept)", "x", "z", "xz"),
...
qr.fit.30 <- lapply(simdata.30, function(d){
  ...
  tryCatch(rq(y~x + z + xz, 
              data=data.frame(d[c("y","x","z","xz")]),
              tau=tau),
           error=function(e) list(as.character(e))) #fit qr model and save error if impossible
})
...
qr.coef.30 <- lapply(1:length(qr.fit.30), function(r){
  ...
  coef.type <- row.names(true_betatau)
  if(is.model.fun(qr.fit.30[[r]], legalmod)){
    df <- data.frame(coef.type, repetition, coef(qr.fit.30[[r]]))
  }else{
    df <- data.frame(coef.type, repetition, matrix(NA, nrow= nrow(true_betatau), ncol= ncol(true_betatau))) #set coef to NA if model could not be estimated
  }
  names(df)[-c(1,2)] <- names(true_betatau)
  df
})
...
qr.se.30 <- lapply(1:length(qr.fit.30), function(r){
  ...
  coef.type <- row.names(true_betatau)
  if(is.model.fun(qr.fit.30[[r]], legalmod)){
    se <- data.frame(sapply(lapply(summary(qr.fit.30[[r]], cov=TRUE, "iid"), "[[", "coefficients"),function(m) m[,"Std. Error"]))
    df <- data.frame(coef.type, repetition, se)
  }else{
    df <- data.frame(coef.type, repetition, matrix(NA, nrow= nrow(true_betatau), ncol= ncol(true_betatau))) #set coef to NA if model could not be estimated
  }
  names(df)[-c(1,2)] <- names(true_betatau)
...
qr.diff.p.30 <- matrix(data=NA, nrow=n_sim,ncol=length(true_betatau))
for (r in 1:n_sim){
  cat("No.", r)
  
  if(is.model.fun(qr.fit.30[[r]], legalmod)){
    qr.diff.p.30[r,] <- anova(qr.sep.fit.30[["tau = 0.25"]][[r]],
                              qr.sep.fit.30[["tau = 0.75"]][[r]],
                              joint="FALSE",
                              se="ker")$table$pvalue
  }else{
    qr.diff.p.30[r,] <- rep(NA, length(true_betatau))
  }
}
...
# Export data.frames for plotting and creating tables
saveRDS(ms.summ.30, file="binary_S7_Lin_ms_summ_30.rds")
saveRDS(qr.est.long.30, file="binary_S7_Lin_est_long_30.rds")
saveRDS(ms.diff.summ.30, file="binary_S7_Lin_diff_ms_summ_30.rds")
saveRDS(qr.diff.agg.long.30, file="binary_S7_Lin_est_agg_long_30.rds")
...
qr.coef.50 <- lapply(1:length(qr.fit.50), function(r){
  ...
  if(is.model.fun(qr.fit.50[[r]], legalmod)){
    df <- data.frame(coef.type, repetition, coef(qr.fit.50[[r]]))
  }else{
    df <- data.frame(coef.type, repetition, matrix(NA, nrow= nrow(true_betatau), ncol= ncol(true_betatau))) #set coef to NA if model could not be estimated
  }
  names(df)[-c(1,2)] <- names(true_betatau)
...
qr.se.50 <- lapply(1:length(qr.fit.50), function(r){
  ...
  coef.type <- row.names(true_betatau)
  if(is.model.fun(qr.fit.50[[r]], legalmod)){
    se <- data.frame(sapply(lapply(summary(qr.fit.50[[r]], cov=TRUE, "iid"), "[[", "coefficients"),function(m) m[,"Std. Error"]))
    df <- data.frame(coef.type, repetition, se)
  }else{
    df <- data.frame(coef.type, repetition, matrix(NA, nrow= nrow(true_betatau), ncol= ncol(true_betatau))) #set coef to NA if model could not be estimated
  }
...
qr.diff.p.50 <- matrix(data=NA, nrow=n_sim,ncol=length(true_betatau))
for (r in 1:n_sim){
  if(is.model.fun(qr.fit.50[[r]], legalmod)){
    qr.diff.p.50[r,] <- anova(qr.sep.fit.50[["tau = 0.25"]][[r]],
                              qr.sep.fit.50[["tau = 0.75"]][[r]],
                              joint="FALSE",
                              se="ker")$table$pvalue
  }else{
    qr.diff.p.50[r,] <- rep(NA, length(true_betatau))
  }
}
...
# Export data.frames for plotting and creating tables
saveRDS(ms.summ.50, file="binary_S7_Lin_ms_summ_50.rds")
saveRDS(qr.est.long.50, file="binary_S7_Lin_est_long_50.rds")
saveRDS(ms.diff.summ.50, file="binary_S7_Lin_diff_ms_summ_50.rds")
saveRDS(qr.diff.agg.long.50, file="binary_S7_Lin_est_agg_long_50.rds")
...
true_betatau <- data.frame(true_betatau, row.names = c("(Intercept)", "x", "z", "xz"),
                           stringsAsFactors = FALSE)
names(true_betatau) <- c("Tau25", "Tau1000", "Tau75")
...
qr.coef.1000 <- lapply(1:length(qr.fit.1000), function(r){
  ...
  if(is.model.fun(qr.fit.1000[[r]], legalmod)){
    df <- data.frame(coef.type, repetition, coef(qr.fit.1000[[r]]))
  }else{
    df <- data.frame(coef.type, repetition, matrix(NA, nrow= nrow(true_betatau), ncol= ncol(true_betatau))) #set coef to NA if model could not be estimated
  }
  names(df)[-c(1,2)] <- names(true_betatau)
...
qr.se.1000 <- lapply(1:length(qr.fit.1000), function(r){
  ...
  coef.type <- row.names(true_betatau)
  if(is.model.fun(qr.fit.1000[[r]], legalmod)){
    se <- data.frame(sapply(lapply(summary(qr.fit.1000[[r]], cov=TRUE, "iid"), "[[", "coefficients"),function(m) m[,"Std. Error"]))
    df <- data.frame(coef.type, repetition, se)
  }else{
    df <- data.frame(coef.type, repetition, matrix(NA, nrow= nrow(true_betatau), ncol= ncol(true_betatau))) #set coef to NA if model could not be estimated
  }
...
qr.diff.p.1000 <- matrix(data=NA, nrow=n_sim,ncol=length(true_betatau))
for (r in 1:n_sim){
  if(is.model.fun(qr.fit.1000[[r]], legalmod)){
    qr.diff.p.1000[r,] <- anova(qr.sep.fit.1000[["tau = 0.25"]][[r]],
                              qr.sep.fit.1000[["tau = 0.75"]][[r]],
                              joint="FALSE",
                              se="ker")$table$pvalue
  }else{
    qr.diff.p.1000[r,] <- rep(NA, length(true_betatau))
  }
}
...
# Export data.frames for plotting and creating tables
saveRDS(ms.summ.1000, file="binary_S7_Lin_ms_summ_1000.rds")
saveRDS(qr.est.long.1000, file="binary_S7_Lin_est_long_1000.rds")
saveRDS(ms.diff.summ.1000, file="binary_S7_Lin_diff_ms_summ_1000.rds")
saveRDS(qr.diff.agg.long.1000, file="binary_S7_Lin_est_agg_long_1000.rds")
